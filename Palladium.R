...
oil_wti <- oil[, c("Date", "PL.F_Close")]
oil_wti$Date <- as.Date(oil_wti$Date, format = "%d/%m/%Y")


oil_xts <- xts(oil_wti$PL.F_Close, order.by = oil_wti$Date)
...
plot(r_xts, main = "Palladium Prices - Log Returns", ylab = "Log return")
...
ggplot(returns_df, aes(x = log_returns)) +
  ...
  labs(title = "Histogram of WTI Log Returns",
...
ggplot(returns_df, aes(x = log_returns)) +
  ...
  labs(title = "Histogram of Palladium Log Returns with Normal Curve",
...
# QQ plot
qqnorm(eps, main = "QQ Plot of Daily Palladium Log Returns"); qqline(eps, col = "red")



# Autocorrelation
par(mfrow =c(2,2))
acf(r_xts,    main = "ACF of Daily Palladium Returns")
pacf(r_xts,   main = "PACF of Daily Palladium Returns")


acf(r_xts^2,  main = "ACF of Squared Daily Palladium Returns")
pacf(r_xts^2, main = "PACF of Squared Daily Palladium Returns")


par(mfrow = c(1,1))
acf(abs(r_xts),  main = "ACF of Absolute Palladium Returns")   
pacf(abs(r_xts), main = "PACF of Absolute Palladium Returns") 
...
plot(oil_xts, main = "Palladium Daily Closing Price", ylab = "Price")
