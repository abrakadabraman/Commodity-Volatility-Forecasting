...
oil_wti <- oil[, c("Date", "NG.F_Close")]
oil_wti$Date <- as.Date(oil_wti$Date, format = "%d/%m/%Y")


oil_xts <- xts(oil_wti$NG.F_Close, order.by = oil_wti$Date)
...
#oil_xts <- xts(oil_wti$CL.F_Close, order.by = oil_wti$Date)
...
# Centered residuals
# Residuals (assume constant mean = 0, following Taylor)
r <- as.numeric(r_xts)
eps <- r                       # ε_t = r_t (no demeaning)
abs_returns <- abs(eps)
squared_returns <- eps^2       # variance proxy y_t
...
ggplot(returns_df, aes(x = log_returns)) +
  ...
  labs(title = "Histogram of Natural Gas Log Returns",
...
ggplot(returns_df, aes(x = log_returns)) +
  ...
  labs(title = "Histogram of Natural Gas Log Returns with Normal Curve",
...
# QQ plots
qqnorm(eps, main = "QQ Plot of Gas Log Returns"); qqline(eps, col = "red")


# Acf pacf
par(mfrow =c(2,2))
acf(r_xts,    main = "ACF of Daily Gas Returns")
pacf(r_xts,   main = "PACF of Gas Returns")


acf(r_xts^2,  main = "ACF of Squared Daily Gas Returns")
pacf(r_xts^2, main = "PACF of Squared Daily Gas Returns")


par(mfrow = c(1,1))
acf(abs(r_xts),  main = "ACF of Absolute Gas Returns")   # <<< CHANGED (was prices)
pacf(abs(r_xts), main = "PACF of Absolute Gas Returns")  # <<< CHANGED (was prices)
...
plot(oil_xts, main = "Natural Gas Closing Prices", ylab = "Price")
