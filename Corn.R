...
oil_wti <- oil[, c("Date", "ZC.F_Close")]
oil_wti$Date <- as.Date(oil_wti$Date, format = "%d/%m/%Y")

View(oil)
oil_xts <- xts(oil_wti$ZC.F_Close, order.by = oil_wti$Date)
...
ggplot(returns_df, aes(x = log_returns)) +
  ...
  labs(title = "Histogram of Corn Log Returns",
...
ggplot(returns_df, aes(x = log_returns)) +
  ...
  labs(title = "Histogram of Corn Log Returns with Normal Curve",
...
#QQ plots
qqnorm(eps, main = "QQ Plot of Daily Corn Log Returns"); qqline(eps, col = "red")


# autocorrelation plots
par(mfrow =c(2,2))
acf(r_xts,    main = "ACF of Daily Corn Returns")
pacf(r_xts,   main = "PACF of Daily Corn Returns")


acf(r_xts^2,  main = "ACF of Squared Daily Corn Returns")
pacf(r_xts^2, main = "PACF of Squared Daily Corn Returns")


par(mfrow = c(1,1))
acf(abs(r_xts),  main = "ACF of Absolute Corn Returns")   
pacf(abs(r_xts), main = "PACF of Absolute Corn Returns") 
...
plot(oil_xts, main = "Corn Daily Closing Price", ylab = "Price")


plot(oil_xts,
     main = "Corn Daily Closing Price",
     ylab = "Price",
     xlim = c(start(oil_xts), end(oil_xts)))
